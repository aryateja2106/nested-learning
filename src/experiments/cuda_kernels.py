...
def fused_titans_update(
    ...
    M_memory_new = torch.bmm(M_memory, decay_matrix) - eta_t.unsqueeze(-1).unsqueeze(-1) * grad_memory
...
def _newton_schulz_iteration(M: torch.Tensor, steps: int, dtype: torch.dtype, device: torch.device) -> torch.Tensor:
...
def parallel_cms_update(
    ...
    for i, (mlp_block, should_update) in enumerate(zip(mlp_blocks, update_mask)):
        if should_update:
...
def benchmark_cuda_operations(
    ...
    k = torch.randn(batch_size, d_model, device=device, dtype=dtype)
    v = torch.randn(batch_size, d_model, device=device, dtype=dtype)

...
from ..core.memory import CMSConfig, ContinuumMemorySystem
from .titans import SelfModifyingTitans, TitansConfig
...
class HopeLayer(nn.Module):
    ...
    def __init__(self, config: HopeConfig, layer_idx: int = 0):
        ...
        if config.use_attention_variant:
            self.seq_mixer = MultiHeadAttention(
                d_model=config.d_model, num_heads=config.num_heads, dropout=config.dropout
            )
        else:
    ...
    def forward(
        ...
        if (
            hasattr(self.seq_mixer, "forward")
            and "attention_mask" in self.seq_mixer.forward.__code__.co_varnames
        ):
...
class Hope(nn.Module):
    ...
    def __init__(self, config: HopeConfig):
        ...
        self.rotary_emb = RotaryEmbedding(dim=config.d_key, max_seq_len=config.max_seq_len)
        ...
        self.layers = nn.ModuleList(
            [HopeLayer(config, layer_idx=i) for i in range(config.num_layers)]
        )

...
class MemoryMLP(nn.Module):
    ...
    def __init__(self, d_input: int, d_output: int, d_hidden: int, dropout: float = 0.1):
        super().__init__()
...
class SelfModifyingTitans(nn.Module):
    ...
    def __init__(self, config: TitansConfig):
        ...
        self.M_k = MemoryMLP(
            config.d_model, config.d_key * config.num_heads, config.d_hidden, config.dropout
        )
        self.M_v = MemoryMLP(
            config.d_model, config.d_value * config.num_heads, config.d_hidden, config.dropout
        )
        ...
        self.M_v_hat_k = MemoryMLP(
            config.d_value * config.num_heads,
            config.d_key * config.num_heads,
            config.d_hidden,
            config.dropout,
        )
        self.M_v_hat_v = MemoryMLP(
            config.d_value * config.num_heads,
            config.d_value * config.num_heads,
            config.d_hidden,
            config.dropout,
        )
        self.M_v_hat_memory = MemoryMLP(
            config.d_value * config.num_heads,
            config.d_value * config.num_heads,
            config.d_hidden,
            config.dropout,
        ...
        self.register_buffer(
            "M_memory", torch.zeros(config.num_heads, config.d_value, config.d_key)
        )
    ...
    def _compute_self_generated_values(self, v: torch.Tensor) -> dict[str, torch.Tensor]:
        """
        Generate values for each memory module (self-modification).

        Equation 84: v̂_□,t = M_□,t-1(v_t)
        """
        return {"k": self.M_v_hat_k(v), "v": self.M_v_hat_v(v), "memory": self.M_v_hat_memory(v)}
    ...
    def forward(self, x: torch.Tensor, return_memory_states: bool = False) -> torch.Tensor:
        """
...
def create_titans(
    d_model: int = 512, d_key: int = 64, d_value: int = 64, num_heads: int = 8, chunk_size: int = 16
) -> SelfModifyingTitans:
    """Factory function for creating Titans with sensible defaults."""
    config = TitansConfig(
        d_model=d_model, d_key=d_key, d_value=d_value, num_heads=num_heads, chunk_size=chunk_size
    )
...
if __name__ == "__main__":
    config = TitansConfig(d_model=256, d_key=32, d_value=32, num_heads=4, chunk_size=8)

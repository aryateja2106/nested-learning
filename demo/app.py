...
try:
    ...
    from src.core.memory import ContinuumMemorySystem, CMSConfig
    from src.models.titans import SelfModifyingTitans, TitansConfig
    from src.models.hope import Hope, HopeConfig
except ImportError:
    print("Warning: Could not import Nested Learning modules. Demo will use placeholder functions.")


def visualize_cms_update_schedule(
    num_levels: int = 4,
    base_chunk_size: int = 8,
    total_steps: int = 64
) -> plt.Figure:
    ...
    for level in range(num_levels):
        ...
        ax.plot([], [], color=colors[level], linewidth=2, 
                label=f'Level {level} (chunk={chunk_size})')
...
def visualize_nested_learning_paradigm() -> plt.Figure:
    ...
    for i, level in enumerate(levels):
        ...
        ax.add_patch(rect)
        ax.text(5, level['y'], level['name'], 
                ha='center', va='center', fontsize=10, fontweight='bold')
    
    # Draw arrows between levels
    for i in range(len(levels) - 1):
        ax.annotate('', xy=(5, levels[i+1]['y'] + 1), xytext=(5, levels[i]['y'] - 1),
                   arrowprops=dict(arrowstyle='->', color='black', lw=2))
    
    # Title
    ax.text(5, 9.5, 'Nested Learning: Multi-Level Optimization', 
...
def create_demo():
    ...
    with gr.Blocks(title="Nested Learning Explorer", theme=gr.themes.Soft()) as demo:
        gr.Markdown("""

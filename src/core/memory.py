...
class ContinuumMemorySystem(nn.Module):
    ...
    def __init__(self, config: CMSConfig):
        ...
        if config.learning_rates is None:
            self.learning_rates = [
                0.01 / (2 ** i) for i in range(config.num_levels)
            ]
        else:
            ...
            if self._accumulated_grads[level_idx] is not None:
                ...
                with torch.no_grad():
                    for param, grad in zip(
                        self.mlp_blocks[level_idx].parameters(),
...
if __name__ == '__main__':
    ...
    for level_idx, updates in enumerate(schedule):
        print(f"Level {level_idx} (chunk_size={cms.chunk_sizes[level_idx]}): "
